...
struct Input {
    ...
    /// time step size
    ///
    static constexpr Real dt = 0.00025;

    /// number of time steps for inner loop
    /// total time step Nt = inner_Nt * outer_Nt
    /// simulation time t = dt*Nt
    ///
    static constexpr unsigned inner_Nt = 400;
    ...
    static constexpr auto part_descs
        = std::make_tuple(BiMaxPlasmaDesc({{1, 40, 2}, 10000, TSC, full_f}, 10, .1),
                          BiMaxPlasmaDesc({{-25, 200, 2}, 10000, TSC, full_f}, 1, 1));
