...
class PartSpecies : public Species {
    ...
    std::unique_ptr<RelativisticVDFVariant> vdf;
...
public:
    ...
    Grid<FourCartTensor> equilibrium_mom2{};
    ...
    PartSpecies &operator=(PartSpecies const &) = delete; // this should not be default-ed
    PartSpecies(ParamSet const &params, KineticPlasmaDesc const &desc, std::unique_ptr<RelativisticVDFVariant> vdf);
...
private:
    ...
    void (PartSpecies::*m_collect_part)(Grid<CartVector> &) const;
    ...
    template <long Order>
    void impl_collect_part(Grid<CartVector> &nV) const;
    void impl_collect_all(Grid<Scalar> &n, Grid<CartVector> &nV, Grid<FourCartTensor> &nvv) const;
